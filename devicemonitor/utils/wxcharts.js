...
var config = {
  ...
  colors: ['#90ed7d', '#ff8040','#ff0000', '#ffc0cb',  '#ffff00', '#00ffff', '#8085e9', '#7cb5ec'],
...
function drawLegend(series, opts, config, context) {
  ...
  var legendPosition = opts.legendPositon;
  //console.log('opts.legendPosition=' + legendPosition);
  ...
  legendList.forEach(function(itemList, listIndex) {
    ...
    if (legendPosition == 'right') {
      startX = opts.width - config.legendWidth + config.padding;
      startY = (config.fontSize + marginTop) + padding + marginTop + listIndex * 4 * (shapeHeight);
    } else {
      startX = (opts.width - width) / 2 + padding;
      startY = opts.height - config.padding - config.legendHeight + listIndex * (config.fontSize + marginTop) + padding + marginTop;
    }
    context.setFontSize(config.fontSize);
    itemList.forEach(function(item) {
      switch (opts.type) {
      ...
      if (legendPosition == 'right') {
        ...
        context.fillText(item.name, startX + padding + shapeWidth, startY + 9);
        context.closePath();
        context.stroke();
        startY += shapeHeight;
      } else {
...
var Charts = function Charts(opts, ctrl) {
  //console.log('Charts opts='+JSON.stringify(opts));
  ...
  opts.legendPositon = opts.legendPositon || {};
  ...
  this.context = wx.createCanvasContext(opts.canvasId, ctrl);
